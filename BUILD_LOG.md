...
**Stack:** Node 20 + Express 5 + PostgreSQL (`pg`, raw SQL) + Redis · React 18 + Vite · plain JavaScript, ESM.
...
|---|---|---|---|
| 0 | Foundation and tooling | 7 | 0/7 |
...
|---|---|---|---|---|---|
| P0-1 | npm workspaces: `server/`, `client/`, `shared/`; ESM everywhere | ⬜ | `package.json`, workspace manifests | `npm install` clean | |
...
| P0-3 | `docker-compose.yml`: Postgres 16, Redis 7 (`notify-keyspace-events Ex`), Mailhog | ⬜ | `docker-compose.yml` | `docker compose up` → all healthy | |
...
| P0-7 | CI workflow: install → lint → test with Postgres+Redis services | ⬜ | `.github/workflows/ci.yml` | Green on first push | |

**Exit criteria:** `docker compose up` gives a booting API with Swagger and a passing health check.
...
| P1-4 | Redis client (separate subscriber connection), keyspace events asserted at boot | ⬜ | `server/src/redis/client.js` | `PING` in `/health` | |
...
| P3-5 | Layer 2: Redis `hold:{id}` PX TTL + keyspace-expiry subscriber → release | ⬜ | `redis/keyspaceListener.js` | Release observed <1s after TTL | |
| P3-6 | Layer 3a: BullMQ `hold-expiry` delayed job (`ttl + 2s` grace) | ⬜ | `queue/workers/holdExpiry.worker.js` | Fires with the Redis listener disabled | |
| P3-7 | Layer 3b: `node-cron` 30s reconciler sweeping expired holds | ⬜ | `jobs/holdReconciler.job.js` | Manually stale row swept within 30s | |
| P3-8 | Redis advisory lock as pre-filter (sorted acquire, Lua release), documented as non-authoritative | ⬜ | `redis/lock.js` | Load test shows reduced DB contention | |
| P3-9 | ⭐⭐ **Concurrency proof suite** — all six scenarios from PROJECT_PROMPT §6.6, on testcontainers | ⬜ | `tests/e2e/concurrency.test.js`, `holdExpiry.test.js` | Green in CI, 3 consecutive runs | |
...
| P4-3 | Idempotency middleware + `Idempotency-Key` on confirm | ⬜ | `middleware/idempotency.js` | 20 parallel replays → 1 booking | |
...
| P4-6 | ⭐ Transactional outbox: row written in the booking transaction; drain worker with backoff + DLQ | ⬜ | `queue/workers/outboxDrain.worker.js` | Rolled-back booking sends no email | |
| P4-7 | Nodemailer + EJS templates; QR inline via CID; Mailhog locally | ⬜ | `mail/*` | Mailhog shows a rendering QR | |
...
| P5-2 | Redis sorted-set mirror for O(log n) position; `GET /waitlist/me` | ⬜ | `waitlist.service.js` | Position correct after mid-queue removal | |
...
| P5-6 | ⭐ Cascade: `FOR UPDATE SKIP LOCKED` next in line, `attempt_no + 1`, `MAX_CASCADE` bound | ⬜ | `offers.service.js`, `offerExpiry.worker.js` | 3-deep cascade test; bound releases seats | |
...
| ID | Change | Status | Files touched | Verified by | Commit |
|---|---|---|---|---|---|
| P6-1 | Socket.IO server, JWT handshake auth, Redis adapter | ⬜ | `realtime/io.js` | Two clients, two API instances, both receive | |
| P6-2 | Room helpers + typed emit helpers used by every service | ⬜ | `realtime/rooms.js`, `emit.js` | No raw event strings anywhere | |
| P6-3 | Wire all seven events from PROJECT_PROMPT §10 into the services | ⬜ | holds, bookings, waitlist services | Two-browser manual check | |
...
| P9-3 | Correlation IDs in `pino`; `/health` reports outbox backlog and queue depth | ⬜ | `utils/logger.js`, `health.routes.js` | Trace one request end to end | |
| P9-4 | Graceful shutdown: drain in-flight requests, close pool and workers cleanly | ⬜ | `server/src/index.js` | SIGTERM mid-hold → no corruption | |
...
| P10-2 | `README.md`: quickstart, architecture diagram, env table, mechanism explanations | ⬜ | `README.md` | Clean machine running in under 5 min | |
...
| P10-6 | Deploy API+PG+Redis (Render/Railway) and client (Vercel); migrate, seed, warm | ⬜ | `docs/DEPLOYMENT.md` | Public URL works from a cold phone | |
...
| D-2 | — | Lazy expiry: `expires_at` in the SQL predicate is authoritative | Cron-only release; Redis-only TTL | Correctness must not depend on a worker running. Schedulers materialise and broadcast; they don't decide. |
...
| D-6 | — | Redis lock is a load-shedder, not the correctness boundary | Redlock as primary | A distributed lock cannot out-guarantee a database that already has row locks. Being explicit about this is the point. |
...
| D-11 | — | `testcontainers` over mocks for concurrency tests | `vi.mock` on the pool | A mocked database has no row locks, so a mocked concurrency test proves nothing. |
...
| SMTP provider blocks unverified recipients | High — no ticket email | Verify a sending domain early; keep Mailhog local; document the sandbox limit | P4-7 |
| Redis keyspace notifications off by default on managed hosts | Medium — layer 2 silently dead | Assert the config at boot and log loudly; layers 1 and 3 still hold | P3-5 |
...
- feat(holds): three-layer TTL — SQL predicate, Redis keyspace expiry, BullMQ + cron [P3-5..P3-7]
- test(concurrency): 50-parallel-hold suite on testcontainers; asserts exactly one winner [P3-9]
### Changed
...
|---|---|---|---|---|
| 0 | `phase/0-foundation` | ⬜ | `v0.1.0-phase0` | |
