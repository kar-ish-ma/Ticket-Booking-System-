...
- Use `testcontainers` with **real Postgres and real Redis**. A mocked database has no row locks, so
  a mocked race test proves nothing. If you find yourself reaching for `vi.mock` on the pg pool, stop.
