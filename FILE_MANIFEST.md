...
**Stack:** Node 20 + Express 5 + PostgreSQL (`pg`, raw SQL) + Redis · React 18 + Vite · plain JavaScript, ESM throughout.
...
| `docker-compose.yml` | Postgres 16, Redis 7 (with `notify-keyspace-events Ex`), Mailhog, server, client. One command to a working system. |
...
| `.gitignore` / `.dockerignore` | Excludes `node_modules`, `.env`, `dist`, coverage. |
...
| File | Purpose |
|---|---|
| `ci.yml` | On push/PR: install → lint → unit → e2e with Postgres and Redis service containers. **The concurrency suite runs here** — visible green CI is part of the pitch. |
...
| File | Purpose |
|---|---|
| `src/index.js` | Entry point. Starts the HTTP server, attaches Socket.IO, starts cron jobs and BullMQ workers, registers graceful-shutdown handlers (in-flight holds must not be orphaned on deploy). |
...
| File | Purpose |
|---|---|
| `src/redis/client.js` | ioredis connections. Note: a **separate subscriber connection** — a subscribed client can't run normal commands. Enables `notify-keyspace-events Ex` at boot and logs loudly if the server refuses. |
| `src/redis/lock.js` | ⭐ `SET NX PX` acquire, Lua-script release that checks the token so you can't free someone else's lock, keys acquired in sorted order. Explicitly a load-shedder, **not** the correctness boundary — commented as such. |
| `src/redis/keyspaceListener.js` | ⭐ Subscribes to `__keyevent@0__:expired`, routes `hold:*` keys to `releaseHold()`. **Layer 2** of the TTL design. |
| `src/queue/queues.js` | BullMQ queue definitions: `hold-expiry`, `offer-expiry`, `outbox-drain`. |
| `src/queue/workers/holdExpiry.worker.js` | ⭐ Delayed job → idempotent release. **Layer 3a.** |
| `src/queue/workers/offerExpiry.worker.js` | ⭐ Offer lapse → cascade to the next entry in the queue. |
| `src/queue/workers/outboxDrain.worker.js` | ⭐ Polls `outbox_events`, sends mail, marks SENT/FAILED with exponential backoff, DEAD after 5 attempts. |
| `src/jobs/holdReconciler.job.js` | ⭐ `node-cron` every 30s sweeping `state='HELD' AND expires_at <= now()`. **Layer 3b** — the safety net under the safety net. |
| `src/jobs/offerReconciler.job.js` | Same for lapsed offers whose delayed job was lost. |
| `src/mail/mailer.js` | Nodemailer transport (Mailhog locally, SMTP in prod). Renders EJS → HTML, attaches the QR PNG via CID. |
...
| `src/middleware/idempotency.js` | ⭐ Redis-backed replay cache keyed on the `Idempotency-Key` header. Same key → cached response, never a second booking. |
...
| | `waitlist.service.js` | ⭐ FIFO queue operations, Redis sorted-set mirror, position lookup. |
...
| **health** | `health.routes.js` | DB, Redis, queue depth, outbox backlog. Used by the platform health check. |

### Realtime

| File | Purpose |
|---|---|
| `src/realtime/io.js` | ⭐ Socket.IO server, JWT handshake auth, Redis adapter for multi-instance fan-out. |
| `src/realtime/rooms.js` | Room naming (`show:{id}`, `user:{id}`) in one place. |
| `src/realtime/emit.js` | Typed emit helpers — services call `emitSeatReleased()`, never a raw event string. |
...
| File | Purpose |
|---|---|
| `src/utils/reference.js` | Booking reference generator (`TF-` + Crockford base32, no ambiguous characters). |
...
| File | Purpose |
|---|---|
| `tests/setup/containers.js` | Spins real Postgres + Redis via `testcontainers`, runs migrations. Mocks cannot test races. |
...
| `TESTING.md` | How to run the concurrency suite and how to read its output. |
| `DEPLOYMENT.md` | Render/Railway/Vercel steps, env setup, migrate and seed commands. |
