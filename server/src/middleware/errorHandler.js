 ...
 * `success`.
 *
 * Does NOT own mapping specific domain errors (SeatsUnavailableError, HoldExpiredError, ...) to
 * their own HTTP status and code — those classes don't exist yet (server/src/utils/errors.js is
 * added as each owning module is built, starting Phase 1). Until a domain error class exists and
 * is caught here explicitly, everything that reaches this handler is an unknown failure: logged
 * in full server-side, and reported to the client as a generic 500 that never leaks a stack
