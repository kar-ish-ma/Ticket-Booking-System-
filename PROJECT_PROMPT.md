# TicketFlow — Master Build Prompt
...
You are the lead engineer building **TicketFlow**, a production-grade ticket booking platform for movies and concerts.
...
- **Every hard mechanism gets a test that fails if the mechanism is removed.** If deleting the Redis lock still passes your tests, your tests are wrong.
- **Commit and push after every completed task** (§1.5).
- **Comment for a reader who is learning the system** (§1.6). The owner will defend this design out loud.
...
`// The Redis lock sheds load before the DB under a seat rush. It is NOT the correctness`
`// boundary — the row lock below is. A distributed lock cannot out-guarantee the database.`
...
|---|---|---|
| 1 | **Lazy expiry as the source of truth.** A hold is expired the instant `expires_at <= now()`, evaluated in the SQL predicate itself. The scheduler only *materialises* and *broadcasts*. | Correctness never depends on a cron job firing. If Redis dies and every worker crashes, seats still free themselves. The most sophisticated idea here — lead the write-up with it. |
...
| 3 | **Three-layer TTL**: DB predicate (authoritative) → Redis keyspace expiry (instant UX) → BullMQ delayed job + 30s reconciler (durable sweep). All idempotent. | Defence in depth, and you can demo killing each layer. |
...
|---|---|---|
| Repo | npm workspaces: `server/`, `client/`, `shared/` | One `npm install`, shared constants, no extra tooling |
| Backend | **Express 5** | Native async error handling — no `express-async-errors` wrapper needed |
| Database | **PostgreSQL 16 + `pg` (node-postgres)** | Raw SQL. The locking query is the centrepiece; nothing should hide it |
| Migrations | **`node-pg-migrate`**, SQL-based | Plain SQL up/down files, ordered, tracked in a table |
| Cache / locks / TTL | **Redis 7 (`ioredis`)** | Keyspace notifications, `SET NX PX` locks, sorted sets |
| Jobs | **BullMQ** | Delayed jobs for hold + offer expiry, retries, DLQ |
| Scheduler | **`node-cron`** | The 30s reconciler |
| Realtime | **Socket.IO** + Redis adapter | Rooms per show, scales horizontally |
| Validation | **Zod** (works fine in JS) | One schema shared by server and client |
| Auth | `jsonwebtoken` + `argon2` + `cookie-parser` | Access 15m / refresh 7d, httpOnly cookies |
| Frontend | **React 18 + Vite + React Router 6** | Fast, familiar, no framework overhead |
| Client state | TanStack Query + Zustand | Query for server cache, Zustand for seat selection |
| Styling | **Tailwind + Headless UI + lucide-react** | Hand-rolled components; no TS-first component library |
| Charts | Recharts | Organiser revenue dashboard |
| Email | **Nodemailer** (Mailhog local, Resend/Brevo in prod) + **EJS** templates | No JSX on the server |
| QR | `qrcode` | PNG buffer, attached inline via CID |
| Testing | **Vitest** + Supertest + `testcontainers` | Concurrency tests need a real DB, not mocks |
| API docs | `swagger-jsdoc` + `swagger-ui-express` | JSDoc comments become live docs at `/api/docs` |
| Logging | `pino` + `pino-http` | Structured logs with request ids |
| Deploy | Render/Railway (API + PG + Redis), Vercel/Netlify (client) | Free tiers |

`docker-compose.yml` must bring up Postgres, Redis, Mailhog, server and client with one command. Assume the grader has Docker and nothing else.

**JSDoc everywhere.** Since there's no compiler, JSDoc is your type documentation. Add `// @ts-check` at the top of `holds`, `bookings`, `waitlist` and `offers` service files and keep a `jsconfig.json` with `checkJs: true` — you get editor-level type checking on the files that matter, with zero build cost.
...
CREATE TABLE bookings (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  reference       text UNIQUE NOT NULL,          -- 'TF-7K2M9QX4'
...
Every read and every write predicate carries `AND (state <> 'HELD' OR expires_at > now())`. The scheduler's job is only to *materialise* the logical truth into a stored row and *broadcast* it. If Redis is down and every worker is dead, the system is still **correct** — just less live.

### 5.2 Three layers

**Layer 1 — Database (authoritative).** `expires_at` on `show_seats`. Availability queries and the acquire predicate both treat an expired hold as available. Nothing to run, nothing to fail.

**Layer 2 — Redis keyspace expiry (instant UX).** On hold creation: `SET hold:{holdId} {showId} PX {ttlMs} NX`. Enable `notify-keyspace-events Ex`. A subscriber on `__keyevent@0__:expired` catches the key the moment it dies, calls `releaseHold(holdId, 'TTL')`, and emits `seat.released` over the socket. Sub-second UI update, no polling.

**Layer 3 — BullMQ delayed job + `node-cron` reconciler (durability).** On hold creation, enqueue `hold-expiry` with `delay = ttlMs + 2000`. Additionally, every 30s:
...
- **Redis advisory lock** (`SET lock:show:{showId}:seat:{seatId} {reqId} NX PX 5000`, keys acquired in sorted order, released via a Lua script that checks the token) as a fast pre-filter. It sheds load before the DB; it is **not** the correctness boundary. Say so — reviewers respect knowing which layer is authoritative.
...
Run against real Postgres + Redis via `testcontainers`. Mocks prove nothing here.
...
Postgres is the source of truth; mirror into a Redis sorted set `waitlist:{showId}:{categoryId}` scored by `enqueued_at` epoch ms for O(log n) "you are #7 of 23".
...
Customer cancels booking
  └─ TX: booking→CANCELLED, payment→REFUNDED, freed seats grouped by category_id
      ...
      └─ after COMMIT: enqueue BullMQ `offer-expiry` delayed job
```
...
GET    /health                            → { db, redis, queue, outboxBacklog }
```
...
- [ ] `README.md` — 60-second quickstart (`docker compose up`), architecture diagram, env table, API reference, DB schema/ERD, and prose on hold TTL, concurrency, and waitlist logic
...
- [ ] File header, JSDoc blocks, and WHY-comments written (§1.6)
