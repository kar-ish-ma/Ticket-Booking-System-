...
| | `offers.service.js` | 🔒⭐ Built at P5-3 with `createInitialOffer()` only — the D-40 `BOOKED -> OFFER_RESERVED` call site (D-48 records why it landed here rather than in a function literally named `cascadeOffer()`: this is the FIRST offer, attempt_no 1, conceptually distinct from P5-6's `OFFER_RESERVED -> OFFER_RESERVED` cascade re-offers). Called from inside `bookings.service.js#cancelBooking`'s own transaction, never opens one of its own. Token is a placeholder (D-49, "Phase 5 debt") until P5-4 builds the real HMAC raw+hash shape; single-use validation and the `MAX_CASCADE` cascade bound are P5-5/P5-6, not this task. |
| | `offers.queries.js` | ⭐ Built at P5-3 with `transitionBookedSeatsToOfferReserved()` (the `BOOKED -> OFFER_RESERVED` half of §7.2, scoped per category so a multi-category booking can route each group independently) and `insertWaitlistOffer()`. Both compute `expires_at`/`reserved_until` via independent `now() + make_interval(...)` expressions rather than one JS timestamp threaded through both calls — D-47 records why that's safe: Postgres's `now()` is fixed for the whole transaction, not per-statement. |
