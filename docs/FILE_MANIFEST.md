...
| `schemas/waitlist.schema.js` | ⭐ Built at P5-1 with `joinWaitlistSchema` only (`{ categoryId, quantity }`) — same split as `hold.schema.js`'s own header: how many seats a join may request in TOTAL isn't capped here (that's server-only `MAX_SEATS_PER_BOOKING` env, enforced in `waitlist.service.js#joinWaitlist`). Position, offer detail, and accept/decline shapes land with P5-2/P5-5. |
...
| `tests/e2e/waitlist.test.js` | ⭐ Built at P5-1: join succeeds only once effective availability is zero (and the row count proves rejection writes nothing); a second join by the same user → `409 ALREADY_WAITLISTED`; and — the addition requested during P5-1's review — position renumbers over a gap: 2 users join, the first is flipped to `CANCELLED` directly via SQL (no `DELETE /waitlist` endpoint exists yet), a 3rd user joins and lands at position 2 of 2, not 3 of 3, proving `ROW_NUMBER()` is computed over the live `WAITING` set rather than a stored counter. Cancel → offer → accept, cascade, and the 10-way offer race are P5-3 onward, not buildable yet (no offers module exists). Owns §6.6's "10 waitlisted users racing one offer" scenario once that module exists — not buildable at P3-9 or P5-1. |
