...
| | `bookings.service.js` | 🔒⭐ Built at P4-2 (`confirmBooking()`), grew at P4-8 (`cancelBooking()`) — both of CLAUDE.md's named hard mechanisms this module owns, each carrying its own numbered WALKTHROUGH comment. `confirmBooking()`: the happy path, the D-40 `assertTransition(HELD, BOOKED)` call site (trivially true today, will matter once Phase 5's offer-accept generalises the same shape), and what the loser of BOTH a double-submit race and a confirm-vs-`DELETE /holds/:id` race experiences. `cancelBooking()`: the SAME unconditional-calls idempotency style as `holds.service.js#releaseHold()` (P3-4) — `markBookingCancelled()`/`refund()`/`releaseBookedSeats()` all run every call, each one's own predicate is what makes a repeat call a no-op — plus a THIRD `assertTransition(BOOKED, AVAILABLE)` call site (D-45, extending D-40/D-44's reasoning) and the `Map<categoryId, seats[]>` grouping §7.2/D-7 requires so Phase 5's P5-3 can insert its `OFFER_RESERVED` branch per category without restructuring anything here. Two inline placeholder generators (`generatePlaceholderReference()`, `generatePlaceholderQrToken()`) stand in for P4-4/P4-5, deliberately skipped — see "Phase 4 debt" below. |
| | `bookings.queries.js` | 🔒⭐ Built at P4-2 (`confirmHeldSeats()` — the literal §6.5 atomic `UPDATE`, no `FOR UPDATE` CTE needed the way `acquireSeats()` needs one, since every row a call could touch is already scoped to one `hold_id`), grew at P4-8 with `releaseBookedSeats()` (the §7.2 counterpart — same single-statement, no-read-then-write shape, `RETURNING category_id` specifically so the service layer can group without a second query), `markBookingCancelled()` (predicate-is-the-idempotency, same idiom as `holds.queries.js#markSeatHoldReleased`), and `findBookingById()` (the one read-only function accepting a bare `pool`, for `loadBookingForOwnership`'s route-middleware use). Also `findHoldSeats()` (deliberately not `FOR UPDATE` — the guarded `UPDATE` is what enforces correctness) and `insertBooking()`/`insertBookingSeats()` (the latter a `unnest()` bulk insert, same shape as `venues.queries.js#insertSeatsBulk`). |
...
| `tests/e2e/bookingCancel.test.js` | ⭐ Built at P4-8. Happy path: cancel refunds the payment, releases the seat, and the response groups it under its `categoryId`. Idempotent double-cancel: second call → `cancelled:false`, exactly one `REFUNDED` payment row — proving composition with P4-1's already-falsified `refund()` guard, not just trusting it by inspection. Non-owner → `403`, nothing changed. Falsified live: dropped `markBookingCancelled()`'s `AND status = 'CONFIRMED'` guard, the double-cancel test's second-call assertion correctly failed (`cancelled:true` instead of `false`); restored. Every released seat here goes to `AVAILABLE` — the `OFFER_RESERVED` branch is Phase 5's P5-3, not yet buildable (no waitlist entries can exist until P5-1 lands). |
