...
| `tests/e2e/holdExpiry.test.js` | ⭐ Built at P3-9. A `HELD` seat manually rewound past its `expires_at`, with zero schedulers of any kind running in this codebase (Layers 2/3 are deferred — see the Phase 3 table) — `GET /shows/:id/seatmap` still reports `AVAILABLE`. Proves Layer 1 alone, not some other layer masking a broken one. |
