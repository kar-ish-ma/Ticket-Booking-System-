...
| **bookings** | `bookings.routes.js`, `bookings.controller.js` | Built at P4-2: `POST /confirm` only (`requireAuth` + `validate` + `requireOwnership`, reading `holdId` from the body — see `bookings.service.js#loadHoldForOwnershipFromConfirmBody`). List / detail / cancel / ticket land later (P4-8's cancellation half next, then P4-9 for history/detail/PDF, both deferred until after Phase 5 — see `docs/BUILD_LOG.md`'s Phase 4 ordering note). |
| | `bookings.service.js` | 🔒⭐ Built at P4-2. `confirmBooking()` — one of CLAUDE.md's four named hard mechanisms, carrying its own numbered WALKTHROUGH comment: the happy path, the D-40 `assertTransition(HELD, BOOKED)` call site (trivially true today, will matter once Phase 5's offer-accept generalises the same shape), and what the loser of BOTH a double-submit race and a confirm-vs-`DELETE /holds/:id` race experiences — composing with P3-4's already-idempotent `releaseHold()` with no new code on either side. Two inline placeholder generators (`generatePlaceholderReference()`, `generatePlaceholderQrToken()`) stand in for P4-4/P4-5, which were deliberately skipped for this task — see "Phase 4 debt" below. |
| | `bookings.queries.js` | 🔒⭐ Built at P4-2. `confirmHeldSeats()` — the literal §6.5 atomic `UPDATE`, carrying its own numbered WALKTHROUGH (no `FOR UPDATE` CTE needed the way `acquireSeats()` needs one: every row a call could touch is already scoped to one `hold_id`, so there's no cross-transaction lock-ordering deadlock to prevent). Also `findHoldSeats()` (the pre-transition read — deliberately not `FOR UPDATE`, since the guarded `UPDATE` is what actually enforces correctness) and `insertBooking()`/`insertBookingSeats()` (the latter a `unnest()` bulk insert, same shape as `venues.queries.js#insertSeatsBulk`). |
