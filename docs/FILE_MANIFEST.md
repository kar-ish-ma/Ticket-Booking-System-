...
| `src/config/env.js` | 🔒 Zod-validated environment. Exits the process at boot (readable error via `z.prettifyError`, not a raw stack) on anything missing or invalid — including a `QR_SIGNING_SECRET` that collides with either JWT secret (D-10). Mirrors `.env.example` exactly; loads `.env` from the repo root via Node's built-in `process.loadEnvFile()`, no `dotenv` dependency needed. |
