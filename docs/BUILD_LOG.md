...
| P0-7 | CI workflow: install → lint → test against GitHub's Postgres service container (Linux runners only — never the dev machine) | ✅ | `.github/workflows/ci.yml`, `docs/FILE_MANIFEST.md` | YAML parsed and validated with `js-yaml` before pushing (not just eyeballed). `npm ci`, `npm run lint`, and `npm test --if-present` all run locally with the exact commands the workflow uses — all exit 0. **Actual CI run not self-verified**: no `gh` CLI in this environment, so "green on first push" needs confirmation from GitHub's Actions tab after this push — asked the user to check. | |
